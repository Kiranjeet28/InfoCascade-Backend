...
exports.register = async (req, res, next) => {
  try {
    const { name, email, urn, crn, password, department, semester, group } =
      req.body;
    ...
    if (!/@gmail\.com$/i.test(email)) {
      return fail(
        res,
        400,
        "STUDENT_REGISTER_INVALID_EMAIL",
    ...
    if (existingStudent) {
      ...
      if (existingStudent.urn === urn) {
        return fail(
          res,
          400,
          "STUDENT_URN_ALREADY_EXISTS",
          "URN is already registered",
          {
            field: "urn",
          },
        );
      ...
      if (existingStudent.crn === crn) {
        return fail(
          res,
          400,
          "STUDENT_CRN_ALREADY_EXISTS",
          "CRN is already registered",
          {
            field: "crn",
          },
        );
  ...
  try {
    ...
    if (!["email", "urn", "crn"].includes(normalizedType)) {
      return fail(
        res,
        400,
        "STUDENT_AVAILABILITY_INVALID_TYPE",
        "type must be one of: 'email', 'urn', 'crn'",
        {
          field: "type",
        },
      );
    ...
    if (!normalizedValue) {
      return fail(
        res,
        400,
        "STUDENT_AVAILABILITY_EMPTY_VALUE",
  ...
  try {
    ...
    if (!identifier || !password) {
      return fail(
        res,
        400,
        "STUDENT_SIGNIN_REQUIRED_FIELDS",
    ...
    if (!student) {
      return fail(
        res,
        400,
        "STUDENT_SIGNIN_INVALID_CREDENTIALS",
        "Invalid credentials",
      );
    }
  ...
  try {
    ...
    if (!urn || !password) {
      return fail(
        res,
        400,
        "STUDENT_AUTH_REQUIRED_FIELDS",
        "urn and password required",
        {
    ...
    if (!student)
      return fail(res, 404, "STUDENT_NOT_FOUND", "Student not found", {
        field: "urn",
      });
    ...
    if (!ok)
      return fail(
        res,
        400,
        "STUDENT_AUTH_INVALID_CREDENTIALS",
        "Invalid credentials",
      );
  ...
  try {
    ...
    if (!identifier || !newPassword) {
      return fail(
        res,
        400,
        "STUDENT_FORGOT_PASSWORD_REQUIRED_FIELDS",
    ...
    if (email) {
      ...
      if (!isVerified) {
        return fail(
          res,
          403,
          "STUDENT_EMAIL_NOT_VERIFIED",
          "Email must be verified via OTP before resetting password",
          {
            field: "email",
          },
        );
